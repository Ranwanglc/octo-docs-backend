...
| `PPT_RELAY_MAX_FRAMES_PER_WINDOW` / `PPT_RELAY_RATE_WINDOW_MS` | no (`200` / `10000`) | sliding-window rate limit on persisted frames (`rate-limited`, the only retryable refusal). |
| `PPT_RELAY_MAX_SINGLE_BLOB_BYTES` | no (`8388608`) | per-snapshot blob cap; larger than the op-frame cap so a legitimate snapshot is not pre-empted by it (`too-large`, permanent). |
